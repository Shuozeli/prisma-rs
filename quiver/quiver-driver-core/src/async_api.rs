...
pub trait Connection: Send + Sync {
    ...
    fn execute(
        &self,
        stmt: &Statement,
    ) -> impl Future<Output = Result<u64, QuiverError>> + Send;
    ...
    fn query_one(
        &self,
        stmt: &Statement,
    ) -> impl Future<Output = Result<Row, QuiverError>> + Send {
        async {
...
pub trait Transactional: Connection {
    ...
    fn begin(
        &mut self,
    ) -> impl Future<Output = Result<Self::Transaction<'_>, QuiverError>> + Send;
}
...
pub trait Driver: Send + Sync {
    ...
    fn connect(
        &self,
        url: &str,
    ) -> impl Future<Output = Result<Self::Conn, QuiverError>> + Send;
}
...
pub trait DynConnection: Send + Sync {
    ...
    fn dyn_execute<'a>(
        &'a self,
        stmt: &'a Statement,
    ) -> BoxFuture<'a, Result<u64, QuiverError>>;
    ...
    fn dyn_query_one<'a>(
        &'a self,
        stmt: &'a Statement,
    ) -> BoxFuture<'a, Result<Row, QuiverError>>;
