...
impl Driver for SqliteDriver {
    ...
    async fn connect(&self, url: &str) -> Result<Self::Conn, QuiverError> {
        ...
        .await
        .map_err(adbc_err)?;
        let conn = adbc::Database::new_connection(&db).await.map_err(adbc_err)?;
...
impl Transactional for SqliteConnection {
    ...
    async fn begin(&mut self) -> Result<Self::Transaction<'_>, QuiverError> {
        adbc::Connection::set_option(
            self.conn.as_ref(),
            ConnectionOption::AutoCommit(false),
        )
        .await
        .map_err(adbc_err)?;
...
mod tests {
    ...
    async fn float_roundtrip() {
        ...
        let row = conn
            .query_one(&stmt("SELECT v FROM t", &[]))
            .await
            .unwrap();
