...
pub async fn introspect(conn: &dyn DynConnection, dialect: SqlDialect) -> Result<Schema, QuiverError> {
    match dialect {
...
async fn list_pg_columns(conn: &dyn DynConnection, table: &str) -> Result<Vec<PgColumn>, QuiverError> {
...
async fn list_pg_primary_keys(conn: &dyn DynConnection, table: &str) -> Result<Vec<String>, QuiverError> {
    let stmt = Statement::new(
...
async fn list_pg_indexes(conn: &dyn DynConnection, table: &str) -> Result<Vec<PgIndex>, QuiverError> {
    let stmt = Statement::new(
...
async fn list_mysql_columns(conn: &dyn DynConnection, table: &str) -> Result<Vec<MysqlColumn>, QuiverError> {
    let stmt = Statement::new(
...
async fn list_mysql_primary_keys(conn: &dyn DynConnection, table: &str) -> Result<Vec<String>, QuiverError> {
    let stmt = Statement::new(
        "SELECT kcu.column_name \
         FROM information_schema.key_column_usage kcu \
         JOIN information_schema.table_constraints tc \
...
async fn list_mysql_indexes(conn: &dyn DynConnection, table: &str) -> Result<Vec<MysqlIndex>, QuiverError> {
    let stmt = Statement::new(
...
async fn sqlite_pk_columns(conn: &dyn DynConnection, table: &str) -> Result<Vec<String>, QuiverError> {
    validate_safe_ident(table)?;
    let stmt = Statement::sql(format!("PRAGMA table_info(\"{}\")", table));
    let rows = conn.dyn_query(&stmt).await?;
...
async fn sqlite_indexes(conn: &dyn DynConnection, table: &str) -> Result<Vec<SqliteIndex>, QuiverError> {
    validate_safe_ident(table)?;
...
async fn sqlite_unique_columns(conn: &dyn DynConnection, table: &str) -> Result<Vec<String>, QuiverError> {
    validate_safe_ident(table)?;
    let stmt = Statement::sql(format!("PRAGMA index_list(\"{}\")", table));
    let rows = conn.dyn_query(&stmt).await?;
...
async fn get_create_sql(conn: &dyn DynConnection, table: &str) -> Result<Option<String>, QuiverError> {
    let stmt = Statement::new(
...
async fn read_enum_values(conn: &dyn DynConnection, enum_table: &str) -> Result<Vec<String>, QuiverError> {
